...
function Answer(text, name, date, score = 0) {
    ...
    this.toString = () => {
        return `${this.name} replied ${this.text} on ${this.date.format('YYYY-MM-DD')} with a score of ${this.score}`;
    }
}
...
function Question(text, name, date) {
    ...
    this.findAll = (name) => this.answers
        .filter(a => a.name === name);

    this.afterDate = (date) => this.answers
        .filter(a => a.date.isAfter(date));
    ...
    this.toString = () => {
        return `Question ${this.name} asked by ${this.text} on ${this.date.format('YYYY-MM-DD')}.
                    It received ${this.answers.length} answers so far: ${this.answers}.`;
    }
}

const question = new Question('Is JS better than python?', 'Mario Rossi', '2023-02-07');
const firstAnswer = new Answer('Yes', 'Luca', '2023-02-15');
const secondAnswer = new Answer('Not in a million year', 'Guido van Rossum', '2023-03-02');
